...
function productShape(product: Product, x: number, y: number) {
  const label = product.title.split(" ").slice(0, 3).join(" ");
  return `
...
export function buildAssetSvg(type: Asset["type"], product: Product) {
  if (type === "pinterest_card") {
    return shell(
      ...
      <text x="120" y="190" font-family="Inter, Arial" font-size="70" font-weight="800" fill="${palette.ink}">${product.title}</text>
      <text x="120" y="270" font-family="Inter, Arial" font-size="42" fill="${palette.ink}">${product.niche} fix under ${Math.round(product.price + 1)}</text>
  ...
  if (type === "tiktok_storyboard") {
    return shell(
      ...
      ${[0, 1, 2, 3].map((row) => `<rect x="80" y="${190 + row * 390}" width="920" height="300" rx="34" fill="#ffffff" stroke="${palette.ink}" stroke-width="4"/><text x="130" y="${265 + row * 390}" font-family="Inter, Arial" font-size="40" font-weight="700" fill="${palette.ink}">${["Problem", "Place it", "Proof shots", "CTA + disclosure"][row]}</text><rect x="130" y="${310 + row * 390}" width="250" height="120" rx="20" fill="${[palette.coral, palette.gold, palette.mint, palette.blue][row]}"/></text>`).join("")}
      <text x="540" y="1810" text-anchor="middle" font-family="Inter, Arial" font-size="34" fill="${palette.ink}">${product.title}</text>
      `,
    );
  }
  ...
  if (type === "comparison_graphic") {
    return shell(
      ...
      <text x="585" y="375" text-anchor="middle" font-family="Inter, Arial" font-size="28" fill="${palette.ink}">${product.niche}</text>
  ...
  if (type === "before_after") {
    return shell(
      ...
      <text x="600" y="820" text-anchor="middle" font-family="Inter, Arial" font-size="34" fill="${palette.ink}">${product.title}</text>
      `,
    );
  }
  ...
  return shell(
    ...
    <text x="1150" y="440" text-anchor="middle" font-family="Inter, Arial" font-size="32" font-weight="800" fill="${palette.ink}">${product.niche}</text>
    `,
  );
}
